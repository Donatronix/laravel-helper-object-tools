...
class Filetotext
{
    public int $multibyte = 4;
    public int $convertquotes = ENT_QUOTES; // Use setUnicode(TRUE|FALSE)
    public bool $showprogress = true; // ENT_COMPAT (double-quotes), ENT_QUOTES (Both), ENT_NOQUOTES (None)
    public string $decodedtext = ''; // TRUE if you have problems with time-out
    ...
    public function convertToText(): array|bool|string|null
    {
        ...
        $file_ext = pathinfo($this->filename, PATHINFO_EXTENSION)['extension'];
    ...
    private function read_doc(): array|string|null
    {
    ...
    private function decodePDF(): void
    {
        ...
        $objects = @$objects[1];
        foreach ($objects as $iValue) {
            $currentObject = $iValue;
    ...
    private function getObjectOptions($object): array
    {
        $options = [];
    ...
    {
        foreach ($textContainers as $j => $jValue) {
    ...
    private function output(bool $echo = false)
    {
        if ($echo) {
    ...
    private function setUnicode($input): void
    {
        if ($input === true) {
            $this->multibyte = 4;
        } else {
